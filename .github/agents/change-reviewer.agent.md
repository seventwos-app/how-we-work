...
These are knowledge and tooling repositories, not applications. The surfaces you will actually see:

| Surface | Typically | Risk |
|---|---|---|
| `*.md` bundle content | OKF v0.2 concepts, frontmatter-carrying | Medium — claim integrity |
| `index.md`, `log.md` | Reserved OKF files with special rules | Medium — validator contract |
| `scripts/**/*.py` | Guards and validators | High — a broken guard fails open |
| `.github/workflows/**` | Required `lint` / `test` / `security` checks | High — privilege and supply chain |
| `.github/agents/**`, `.github/skills/**`, `.copilot/skills/**` | Executable instructions | **Critical** — injection surface |
| `.seventwos/provenance.json` | Provenance records | High — drift makes it decorative |
