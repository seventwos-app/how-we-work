...
| 8 | Agent Gateway Matrix Boundary | No separate Matrix Application Service is part of the Seventwos architecture. A Matrix backend is deployed, and the Agent Gateway — a VM-hosted service, not disclosed further here — connects to it directly and reaches frontier models through Azure AI Foundry; its exact protocol, authentication, and E2EE implementation are not yet documented in this specification. | `[DIRECTIONAL]` | Seventwos architecture direction | 3/5 |
