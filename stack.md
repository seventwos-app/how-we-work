...
- **Agent Gateway**:
  - A Matrix backend is deployed, and the Agent Gateway connects to it directly; the exact client authentication, event-consumption, and encryption mechanism this connection uses remain to be documented in this specification. The gateway is VM-hosted rather than a serverless function app. Its operational surface (deployment, dashboard access, and administration) is tracked in the [`about_hermes`](https://github.com/seventwos-app/about_hermes) repository; its internal implementation details are not disclosed in this specification.
